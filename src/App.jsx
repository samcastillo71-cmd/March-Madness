  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ position: 'relative', height: 140, borderRadius: '12px 12px 0 0', overflow: 'hidden', background: 'linear-gradient(135deg,#0d2818,#1a3a2a)', marginBottom: 0 }}>
        {bannerUrl && !bannerErr && (
          <img src={bannerUrl} alt={teamName} onError={() => setBannerErr(true)} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.4 }} />
        )}
        ...
        <div style={{ position: 'absolute', bottom: 16, left: 20, display: 'flex', alignItems: 'flex-end', gap: 16 }}>
          {logoUrl && !logoErr ? (
            <img src={logoUrl} alt={teamName} onError={() => setLogoErr(true)} style={{ width: 72, height: 72, borderRadius: '50%', background: '#fff', padding: 4, boxShadow: '0 4px 16px rgba(0,0,0,0.5)', objectFit: 'contain' }} />
          ) : (
    ...
    <div style={{ marginBottom: 20 }}>
      <div style={{ position: 'relative', height: 160, borderRadius: '12px 12px 0 0', overflow: 'hidden', background: `linear-gradient(135deg, ${bgDark} 0%, ${bgLight} 100%)` }}>
        {phyloPicUrl && !imgErrors['phylopic'] && (
          <img src={phyloPicUrl} alt={`${animalName} silhouette`} onError={() => handleImgError('phylopic')}
        ...
        {wikiImageUrl && !imgErrors['wiki-header'] && (
          <img src={wikiImageUrl} alt={animalName} onError={() => handleImgError('wiki-header')}
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px 20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          {isAdmin && (
            <button onClick={() => onGenerate(animalName)} disabled={generating} aria-label={`Generate facts for ${animalName}`}
                ...
                <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 8 }}>
                  {galleryImages.map((img, i) => !imgErrors[`gallery-${i}`] && (
                    <div key={i} style={{ flexShrink: 0, textAlign: 'center' }}>
                      <img src={img.url} alt={`${animalName} - ${img.source}`} onError={() => handleImgError(`gallery-${i}`)}
            ...
            {lightbox && (
              <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out', padding: 20 }}
                ...
                <div onClick={e => e.stopPropagation()} style={{ position: 'relative', maxWidth: '90vw', maxHeight: '90vh' }}>
                  <img src={lightbox.url} alt={lightbox.name} style={{ maxWidth: '90vw', maxHeight: '85vh', objectFit: 'contain', borderRadius: 8, display: 'block', boxShadow: '0 8px 40px rgba(0,0,0,0.8)' }} />
                  <div style={{ textAlign: 'center', marginTop: 8, fontSize: 12, color: '#888' }}>{lightbox.source}</div>
                  <button onClick={() => setLightbox(null)} style={{ position: 'absolute', top: -12, right: -12, background: 'rgba(0,0,0,0.7)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '50%', width: 28, height: 28, color: '#fff', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }} aria-label="Close">×</button>
                </div>
              </div>
            )}
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: 14, border: '1px solid rgba(255,255,255,0.07)', gridColumn: '1 / -1' }}>
  ...
  for (const url of urls) {
    try {
      ...
      if (res.ok) {
        ...
        const hasTeams = d.bracket || d.rounds || d.groups || d.regions || (d.events && d.events.length > 0);
...
async function callAI(prompt, _sources = [], textOnly = false) {
  ...
  while (true) {
    ...
    if (res.status === 429) {
      ...
      if (isDaily) throw new Error('Daily quota reached. Try again tomorrow.');
...
function SourcesPanel({ sources, onChange, label }) {
  ...
  return (
    <div style={{ ...S.card, marginBottom: 16, borderColor: 'rgba(99,102,241,0.25)' }}>
      ...
      <p style={{ color: '#777', fontSize: 12, marginBottom: 14 }}>URLs the AI will read before generating research. Primary sources are prioritized.</p>
      {sources.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
          {sources.map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: '8px 12px', border: '1px solid rgba(255,255,255,0.07)' }}>
              <button onClick={() => toggle(i)} style={{ ...S.btn(s.primary ? '#6366f1' : 'rgba(255,255,255,0.08)', s.primary ? '#fff' : '#888'), padding: '3px 10px', fontSize: 10, flexShrink: 0 }}>
              ...
              <button onClick={() => remove(i)} style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: 16, padding: '0 4px', flexShrink: 0 }} aria-label={`Remove ${s.name}`}>×</button>
            </div>
          ))}
        </div>
      )}
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <input placeholder="Source name" value={newName} onChange={e => setNewName(e.target.value)} style={{ ...S.input, flex: 1, minWidth: 160, padding: '7px 12px', fontSize: 12 }} />
        <input placeholder="URL" value={newUrl} onChange={e => setNewUrl(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') add(); }} style={{ ...S.input, flex: 2, minWidth: 200, padding: '7px 12px', fontSize: 12 }} />
        <button style={{ ...S.btn('#6366f1', '#fff'), padding: '7px 16px', fontSize: 12, flexShrink: 0 }} onClick={add}>+ Add Source</button>
      </div>
    </div>
  );
}
...
function LatinNameReviewModal({ review, onConfirm, onCancel }) {
  ...
  const updateLatin = (i, value) => setAnimals(prev => prev.map((a, idx) => idx === i ? { ...a, latinName: value } : a));
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div style={{ ...S.card, maxWidth: 680, width: '100%', maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderColor: 'rgba(134,239,172,0.3)' }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: '#888', fontSize: 13, lineHeight: 1.6 }}>Claude identified the following scientific names. Review and correct any errors before images are fetched.</p>
        </div>
          ...
          {animals.map((animal, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, marginBottom: 4, alignItems: 'center' }}>
              <div style={{ fontSize: 14, color: '#ccc', padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px 0 0 6px', border: '1px solid rgba(255,255,255,0.07)' }}>
                <span style={{ fontSize: 10, color: '#555', marginRight: 6 }}>#{animal.seed}</span>{animal.name}
              </div>
              <input value={animal.latinName} onChange={e => updateLatin(i, e.target.value)} placeholder="Genus species"
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 16 }}>
          <button style={{ ...S.btn('rgba(255,255,255,0.07)', '#888'), padding: '9px 20px' }} onClick={onCancel}>Cancel</button>
          <button style={{ ...S.btn('#16a34a', '#fff'), padding: '9px 24px' }} onClick={() => onConfirm(animals, review.allData)}>Looks Good — Fetch Images →</button>
        </div>
      </div>
    </div>
  );
...
{"summary":"2-3 sentence plain English summary written for a middle schooler. Who is favored and why?","upsetFactors":["Factor 1 — specific reason the underdog could win","Factor 2","Factor 3"],"favoriteStrengths":["Strength 1 of the favorite","Strength 2"],"keyStats":["Stat comparison 1","Stat comparison 2","Stat comparison 3"],"verdict":"1-2 sentence plain English pick with confidence level","upsetOdds":"Low / Medium / High"}`;
    try {
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontFamily: "'Playfair Display', serif", color: ACCENT2, marginBottom: 6, fontSize: 26 }}>🔍 Check My Matchup</h2>
        <p style={{ color: '#999', fontSize: 14, lineHeight: 1.7 }}>
          Click any first-round game to get an AI-powered upset analysis using your team research data{bbSources.length > 0 ? ' and live stats from your research sources' : ''}.
        </p>
      </div>
      {noResearch && (
        <div style={{ ...S.card, textAlign: 'center', padding: 40, color: '#666', marginBottom: 24 }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📋</div>
          <div style={{ fontSize: 15, marginBottom: 6, color: '#888' }}>{matchups.length === 0 ? 'No bracket set up yet' : 'No research data generated yet'}</div>
          <div style={{ fontSize: 13 }}>{matchups.length === 0 ? 'Ask your admin to enter teams and apply the bracket first.' : 'Ask your admin to generate research in Admin → 🏀 Basketball first.'}</div>
        </div>
      )}
      {matchups.length > 0 && (
          ...
          {matchups.map((m, i) => {
            ...
            return (
              <button key={i} onClick={() => handleSelect(m)} disabled={loading}
                style={{ background: isActive ? 'rgba(22,163,74,0.15)' : 'rgba(255,255,255,0.03)', border: isActive ? '2px solid rgba(22,163,74,0.6)' : '1px solid rgba(255,255,255,0.09)', borderRadius: 10, padding: '12px 14px', cursor: loading ? 'wait' : 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all .15s', opacity: loading && !isActive ? 0.5 : 1 }}>
                <div style={{ fontSize: 10, color: regionColor, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>
                  {m.region} Region{!hasResearch && <span style={{ color: '#555', marginLeft: 6, fontWeight: 400 }}>· no research yet</span>}
                </div>
      ...
      {loading && (
        <div style={{ ...S.card, textAlign: 'center', padding: 40, borderColor: 'rgba(99,102,241,0.3)' }}>
          ...
          <div style={{ fontSize: 13, color: '#666' }}>Claude is reading {bbSources.length > 0 ? 'your research sources and ' : ''}the team data</div>
        </div>
      )}
      {error && !loading && (
        <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', fontSize: 13, color: '#f87171', marginBottom: 16 }}>⚠️ {error}</div>
      )}
      {analysis && !loading && selected && (
        <div style={{ ...S.card, borderColor: 'rgba(99,102,241,0.35)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
            <div>
              <div style={{ fontSize: 11, color: '#777', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>{selected.region} Region · First Round</div>
          ...
          {analysis.summary && (
            <div style={{ padding: '14px 16px', background: 'rgba(255,255,255,0.04)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', marginBottom: 16, fontSize: 14, color: '#ccc', lineHeight: 1.8 }}>{analysis.summary}</div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            {analysis.upsetFactors?.length > 0 && (
              <div style={{ background: 'rgba(239,68,68,0.06)', borderRadius: 10, padding: '14px 16px', border: '1px solid rgba(239,68,68,0.2)' }}>
                <div style={{ fontSize: 11, color: '#f87171', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>⚡ Upset Factors</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {analysis.upsetFactors.map((f, i) => (
                    <div key={i} style={{ display: 'flex', gap: 8, fontSize: 13, color: '#ccc', lineHeight: 1.5 }}>
                      <span style={{ color: '#f87171', fontWeight: 700, flexShrink: 0 }}>•</span><span>{f}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {analysis.favoriteStrengths?.length > 0 && (
              <div style={{ background: 'rgba(22,163,74,0.06)', borderRadius: 10, padding: '14px 16px', border: '1px solid rgba(22,163,74,0.2)' }}>
                <div style={{ fontSize: 11, color: ACCENT2, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>🛡️ Favorite's Strengths</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {analysis.favoriteStrengths.map((f, i) => (
                    <div key={i} style={{ display: 'flex', gap: 8, fontSize: 13, color: '#ccc', lineHeight: 1.5 }}>
                      <span style={{ color: ACCENT2, fontWeight: 700, flexShrink: 0 }}>•</span><span>{f}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {analysis.keyStats?.length > 0 && (
            <div style={{ background: 'rgba(99,102,241,0.06)', borderRadius: 10, padding: '14px 16px', border: '1px solid rgba(99,102,241,0.2)', marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: '#a5b4fc', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>📊 Key Stats</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ marginTop: 14, textAlign: 'right' }}>
            <button onClick={() => handleSelect(selected)} style={{ ...S.btn('rgba(255,255,255,0.07)', '#888'), padding: '6px 16px', fontSize: 12 }}>🔄 Re-analyze</button>
          </div>
        </div>
      )}
    </div>
            ...
            {['East','West','South','Midwest'].map(r => (
              <div key={r} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <input value={regionNames[r]} onChange={e => onRegionNamesChange({ ...regionNames, [r]: e.target.value })}
                  aria-label={`${r} region display name`} placeholder={r}
            ...
            <input placeholder="ESPN ID" value={team.espnId} aria-label={`ESPN ID for ${team.name}`}
              ...
              onBlur={async e => {
                ...
                if (fetched) {
                  ...
                  if (!currentName || currentName.startsWith('Seed ')) updateTeam(activeRegion, idx, 'name', fetched);
                }
              }}
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {['East','West','South','Midwest'].map(r => (
              <div key={r} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 11, color: RC[r], fontWeight: 700 }}>{r}:</span>
                <input value={regionNames[r]} onChange={e => onRegionNamesChange({ ...regionNames, [r]: e.target.value })}
                  aria-label={`${r} mammal region display name`} placeholder={r}
                  style={{ ...S.input, width: 120, padding: '4px 8px', fontSize: 12 }} />
              </div>
            ))}
          </div>
...
// This function is defined inside the App component below, but the bracket
...
export default function App() {
  ...
  const [latinReview,          setLatinReview]           = useState(null);
  ...
  useEffect(() => {
    ...
    const u8 = subscribeToMammalResearchData(data => {
      ...
      setMammalSelectedAnimal(prev => { if (prev && data[prev]) return prev; return Object.keys(data)[0] || null; });
    });
  ...
  const handleSaveYear = async () => {
    ...
    setYearSaving(true);
    try { await setDoc(doc(db, 'tournament', 'config'), { year: yr }, { merge: true }); setTournamentYear(yr); }
  ...
  const handleClearBbRoster = useCallback(async () => {
    try {
      await Promise.all([deleteDoc(doc(db, 'admin', 'teamRoster')).catch(() => {}), deleteDoc(doc(db, 'admin', 'officialBracket')).catch(() => {})]);
      setOfficialBracket(null); setBracket(buildInitialBracket());
    } catch (e) { console.warn('Failed to clear bb roster:', e); }
  }, []);

  const handleClearMammalRoster = useCallback(async () => {
    try {
      await Promise.all([deleteDoc(doc(db, 'admin', 'mammalRoster')).catch(() => {}), deleteDoc(doc(db, 'admin', 'officialBracket_mammals')).catch(() => {})]);
      setMammalOfficialBracket(null); setMammalBracket(buildInitialBracketFromTeams(makePlaceholderMammalRoster()));
    } catch (e) { console.warn('Failed to clear mammal roster:', e); }
  }, []);

  const handleClearBbResearch = useCallback(async () => {
    try { await deleteDoc(doc(db, 'admin', 'researchData')).catch(() => {}); setResearchData({}); setSelectedTeam(null); }
    catch (e) { console.warn('Failed to clear bb research:', e); }
  }, []);

  const handleClearMammalResearch = useCallback(async () => {
    try { await deleteDoc(doc(db, 'admin', 'researchData_mammals')).catch(() => {}); setMammalResearchData({}); setMammalSelectedAnimal(null); }
  ...
  const handleClearAllBrackets = useCallback(async (isMammal) => {
    try {
      ...
      const [bracketSnap, lbSnap] = await Promise.all([getDocs(collection(db, colName)), getDocs(collection(db, lbName))]);
      await Promise.all([...bracketSnap.docs.map(d => deleteDoc(d.ref)), ...lbSnap.docs.map(d => deleteDoc(d.ref))]);
  ...
  const saveWithRetry = useCallback(async (saveFn, data, label) => {
    ...
    console.error('Firestore save permanently failed for ' + label);
    return false;
  }, []);
      ...
      if (snap.exists()) {
        ...
        if (d.teams && typeof d.teams === 'object') {
          ...
          Object.entries(d).forEach(([k, v]) => {
            if (k !== 'updatedAt' && typeof v === 'object' && v !== null) allData[k] = v;
          });
        }
      }
    } catch {}
  ...
  const handleFetchMammalImages = useCallback(async (reviewedAnimals, allData) => {
    ...
    for (let i = 0; i < reviewedAnimals.length; i++) {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data.result && allData[name]) allData[name] = { ...allData[name], ...data.result };
        }
  ...
  const handleRefetchMammalImages = useCallback(async (onlyRegion) => {
    ...
    const animals = Object.entries(allData)
      .filter(([, card]) => { if (!card.latinName) return false; if (onlyRegion && card.region !== onlyRegion) return false; return true; })
      .map(([name, card]) => ({ name, latinName: card.latinName }));
    if (!animals.length) { setMammalGenError('No Latin names found for this region. Generate text content first.'); return; }
...
Keep all language at a middle school reading level. Make it engaging and educational. No graphic violence descriptions.`;
    ...
    setMammalGenProgress(prev => ({ ...prev, done: animals.length, current: '' }));
    setMammalGenerating(false);
    if (textOnlyAnimals.length > 0) setLatinReview({ animals: textOnlyAnimals, allData });
  ...
  const renderBracket = (isMammal) => {
    ...
    const SH = 89;
    const FF_SCALE = 1.25;
    const FF_W = Math.round(CW * FF_SCALE);
    const FF_H = Math.round(SH * FF_SCALE);
    ...
    const ScaledGame = ({ children, isHoriz }) => {
      ...
      return (
        <div style={{ width: FF_W, height: wrapH, position: 'relative', overflow: 'visible' }}>
          <div style={{ position: 'absolute', top: 0, left: 0, width: CW, transformOrigin: 'top left', transform: `scale(${FF_SCALE})` }}>
            {children}
          </div>
        </div>
      );
    ...
    const TOTAL_W = CW * 11;
    const RIGHT_LBL_X = CW * 3;
    const LEFT_LBL_X  = CW * 3;
    const FF_CENTER_H = TOP_H;

    const RegionLabel = ({ name, color, isRight, isBottom }) => (
      <div style={{ position: 'absolute', width: CW * 2, height: SH * 2, ...(isRight ? { right: RIGHT_LBL_X } : { left: LEFT_LBL_X }), ...(isBottom ? { top: 0 } : { bottom: 0 }), display: 'flex', alignItems: 'center', justifyContent: isRight ? 'flex-end' : 'flex-start', pointerEvents: 'none', zIndex: 2, overflow: 'hidden' }}>
        <span style={{ fontSize: labelFontSize(name), fontWeight: 900, color, opacity: 0.18, letterSpacing: 2, textTransform: 'uppercase', userSelect: 'none', lineHeight: 1, whiteSpace: 'nowrap' }}>{name}</span>
      </div>
    );
    ...
    const BracketConnectors = ({ dir }) => {
      ...
      const ffCenterX   = CW * 4 + (CW * 3) / 2;
      const ffLeftEdge  = CW * 4 + (CW * 3 - FF_W) / 2;
      const ffRightEdge = ffLeftEdge + FF_W;
      ...
      return (
        <svg width={TOTAL_W} height={H} style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none', zIndex: -1 }} aria-hidden="true">
          {lines}
        </svg>
      );
    };
  ...
  return (
    <ErrorBoundary>
      <div style={S.app}>
        ...
        {latinReview && (
          <LatinNameReviewModal review={latinReview} onConfirm={handleFetchMammalImages} onCancel={() => setLatinReview(null)} />
        )}
        {viewingBracket && (
        ...
        <main style={{ paddingBottom: 60 }}>
              ...
              {researchMatchup && (
                <div style={{ marginBottom: 28 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
                    <button onClick={() => setResearchMatchup(null)} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '6px 14px', color: '#aaa', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>← Back to Browse</button>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 0, alignItems: 'start' }}>
                    <div style={{ borderRadius: '12px 0 0 12px', border: '1px solid rgba(99,102,241,0.3)', overflow: 'hidden' }}>
                      <div style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.2),rgba(99,102,241,0.05))', padding: '10px 16px', borderBottom: '1px solid rgba(99,102,241,0.2)' }}>
                    ...
                    <div style={{ borderRadius: '0 12px 12px 0', border: '1px solid rgba(251,146,60,0.3)', overflow: 'hidden' }}>
                      <div style={{ background: 'linear-gradient(135deg,rgba(251,146,60,0.2),rgba(251,146,60,0.05))', padding: '10px 16px', borderBottom: '1px solid rgba(251,146,60,0.2)' }}>
          ...
          {tab === 'upsetguide' && (
            <div style={{ padding: 24, maxWidth: 860, margin: '0 auto' }}>
              <UpsetGuideMatchupTool researchData={researchData} bbSources={bbSources} officialBracket={officialBracket} />
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 32, marginBottom: 28 }}>
                ...
                <div style={{ padding: '12px 16px', background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.2)', borderRadius: 10, fontSize: 13, color: '#aaa', lineHeight: 1.7 }}>
                  <strong style={{ color: ACCENT2 }}>How to read fractions in this guide:</strong> When you see something like <strong style={{ color: '#fff' }}>8/10 = 0.80</strong>, it means 8 out of 10 times (80% of the time) that thing happened.
                </div>
              </div>
              ...
              <div style={{ ...S.card, marginBottom: 24, borderColor: 'rgba(99,102,241,0.3)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    ['AdjOE', 'Adjusted Offensive Efficiency — points scored per 100 possessions, adjusted for opponent. Higher ranking = better offense.'],
                    ['AdjDE', 'Adjusted Defensive Efficiency — points allowed per 100 possessions. Higher ranking = better defense.'],
                    ['EFG%', 'Effective Field Goal % — shooting percentage that gives extra credit for 3-pointers.'],
                    ['EFGD%', 'Opponent Effective Field Goal % — how well a team limits opponent shooting.'],
                    ['TOR', 'Turnover Rate — how often a team turns it over. Higher ranking = fewer turnovers (good).'],
                    ['TORD', 'Opponent Turnover Rate — how often a team forces opponent turnovers.'],
                    ['ORB', 'Offensive Rebound % — how often a team grabs its own missed shots.'],
                    ['DRB', 'Defensive Rebound % — how often a team grabs opponent missed shots.'],
                    ['3P%', '3-Point % — how well a team shoots from 3-point range.'],
                    ['3PRD', 'Opponent 3-Point Rate — how often opponents shoot 3s against this team.'],
                    ['Tempo', 'How fast a team plays (possessions per game). Low tempo = slow pace.'],
                    ['BARTHAG', 'Overall power rating — basically a grade for how good the whole team is.'],
                  ].map(([abbr, def]) => (
              ...
              <div style={{ ...S.card, marginBottom: 24, borderColor: 'rgba(245,158,11,0.3)' }}>
                ...
                <p style={{ color: '#ccc', fontSize: 14, lineHeight: 1.8 }}>
                  If a 1 seed and a 16 seed played 1,000 games, the 1 seed would win most of them. But in a real tournament, they only play <strong style={{ color: GOLD2 }}>once</strong>. That one game can go wrong for anyone.
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
                  {[['🐢 Slow pace','Fewer total possessions = more variance = underdogs need fewer lucky plays to win'],['🎯 3-point shooting','A hot shooting night from deep can erase a talent gap fast'],['🔄 Turnover battles','If the underdog forces turnovers, they control more possessions than expected'],['🎓 Experienced coaches',"Coaches who've been here before keep their teams calm under pressure"]].map(([icon, text]) => (
                    <div key={icon} style={{ display: 'flex', gap: 10, padding: '8px 12px', background: 'rgba(245,158,11,0.06)', borderRadius: 8, border: '1px solid rgba(245,158,11,0.15)' }}>
                      ...
                      <div><span style={{ color: GOLD2, fontWeight: 700, fontSize: 13 }}>{icon.split(' ').slice(1).join(' ')}: </span><span style={{ color: '#aaa', fontSize: 13 }}>{text}</span></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {[
                { matchup: '1 vs. 16', record: '38/40 = 0.95', color: '#ef4444', scenarios: [{ label: 'Watch out if the 1 seed plays very slowly', detail: "When the 1 seed's tempo ranks outside the top 300, they're only 8/10 = 0.80. Otherwise 30/30 = 1.00.", tip: 'A slow game gives the underdog more chances for lucky plays.' }, { label: 'Watch out if the 16 seed is great at turnovers', detail: "When the 16 seed has both TOR and TORD ranked in the top 70, the 1 seed is only 1/3 = 0.33. Otherwise 37/37 = 1.00.", tip: 'This year: No matchups meet either scenario. Safe to pick all 1 seeds.' }], thisYear: 'No matchups meet either upset scenario this year. All 1 seeds look safe.' },
                { matchup: '2 vs. 15', record: '36/40 = 0.90', color: '#f97316', scenarios: [{ label: 'Watch out if the 2 seed has weak defense', detail: "When the 2 seed's AdjDE ranks outside the top 25, they're only 10/14 = 0.71. With top-25 defense, they're 26/26 = 1.00.", tip: 'Even favorites lose more when their defense is shaky.' }, { label: 'Watch out if the 15 seed shoots inside well AND limits 3s', detail: "When the 15 seed's 2P%D and 3PRD are both in the top 140, the 2 seed is 13/17 = 0.76. Otherwise 23/23 = 1.00.", tip: 'Check your specific matchup this year for these criteria.' }], thisYear: 'Check AdjDE for your 2 seeds. A weak-defense 2 seed is always a slight risk.' },
                { matchup: '3 vs. 14', record: '35/40 = 0.875', color: '#eab308', scenarios: [{ label: 'Watch out if the 3 seed turns it over a lot', detail: "When the 3 seed's TOR ranks outside the top 200, they're only 5/8 = 0.625. Otherwise 30/32 = 0.94.", tip: 'Turnovers give underdogs extra possessions — free chances to score.' }, { label: 'Watch out if the 14 seed forces LOTS of turnovers', detail: "When the 14 seed's TORD is in the top 25, the 3 seed is only 1/4 = 0.25. Otherwise 34/36 = 0.94.", tip: 'A turnover-forcing 14 seed vs a sloppy 3 seed is a real upset alert.' }], thisYear: 'Check your 3 seeds for turnover rate, and your 14 seeds for TORD ranking.' },
                { matchup: '4 vs. 13', record: '32/40 = 0.80', color: '#22c55e', scenarios: [{ label: 'Watch out if the 4 seed plays slowly', detail: "When the 4 seed's tempo ranks outside the top 250, they're only 5/9 = 0.56. Otherwise 27/31 = 0.87.", tip: 'Slow pace again — fewer possessions means more unpredictable outcomes.' }, { label: 'Watch out if the 13 seed shoots really well', detail: "When the 13 seed's EFG% is in the top 40, the 4 seed is 13/18 = 0.72. Otherwise 19/22 = 0.86.", tip: 'An elite-shooting 13 seed is a classic upset threat.' }], thisYear: 'Check tempo for your 4 seeds and shooting efficiency for your 13 seeds.' },
                { matchup: '5 vs. 12', record: '27/40 = 0.675', color: '#06b6d4', scenarios: [{ label: "Watch out if the 5 seed can't force turnovers", detail: "When the 5 seed's TORD ranks outside the top 175, they're only 9/16 = 0.56. Otherwise 18/24 = 0.75.", tip: "A 5 seed that can't create turnovers gives the 12 seed more chances." }, { label: "Watch out if the 12 seed has great shot defense", detail: "When the 12 seed's EFGD% is in the top 50, the 5 seed is only 8/17 = 0.47 — basically a coin flip! Otherwise 19/23 = 0.83.", tip: '5 vs 12 is the most popular upset pick every year for good reason.' }], thisYear: '5 vs 12 upsets happen 1 in 3 times. Always worth at least one in your bracket.' },
                { matchup: '6 vs. 11', record: '19/40 = 0.475', color: '#8b5cf6', scenarios: [{ label: 'Watch out if the 6 seed turns it over a lot', detail: "When the 6 seed's TOR ranks outside the top 140, they're only 3/12 = 0.25. Otherwise 16/28 = 0.57.", tip: 'At this seed, even a small weakness gets punished.' }, { label: "Watch out if the 11 seed's coach has more experience", detail: "When the 11 seed's coach has at least as many Sweet 16 appearances as the 6 seed's coach, the 6 seed is only 7/25 = 0.28. Otherwise 12/15 = 0.80.", tip: '6 seeds actually LOSE more than they win historically — 11 seeds are live underdogs.' }], thisYear: '6 vs 11 is a coin flip. Use turnover rate and coach experience to pick your spots.' },
                { matchup: '7 vs. 10', record: '25/40 = 0.625', color: '#ec4899', scenarios: [{ label: 'Watch out if the 7 seed shoots poorly', detail: "When the 7 seed's EFG% ranks outside the top 120, they're only 7/16 = 0.44. Otherwise 18/24 = 0.75.", tip: 'Struggling to shoot efficiently is a red flag for any favorite.' }, { label: 'Watch out if the 10 seed has elite defense', detail: "When the 10 seed's AdjDE is in the top 30, the 7 seed is only 4/13 = 0.31. Otherwise 21/27 = 0.78.", tip: 'An elite-defense 10 seed vs a poor-shooting 7 seed is a strong upset pick.' }], thisYear: 'Check EFG% for your 7 seeds and AdjDE for your 10 seeds.' },
                { matchup: '8 vs. 9', record: '18/40 = 0.45', color: '#14b8a6', scenarios: [{ label: 'Watch out if the 9 seed plays slower', detail: 'When the 9 seed plays with a slower tempo than the 8 seed, the 8 seed is only 5/21 = 0.24! Otherwise 13/19 = 0.68.', tip: 'Slower tempo favors the underdog in close matchups.' }, { label: "Watch out if the 9 seed's coach has more Sweet 16s", detail: "When the 9 seed's coach has been to more Sweet 16s, the 8 seed is only 5/19 = 0.26. Otherwise 13/21 = 0.62.", tip: '8 vs 9 is basically a coin flip. Tempo and coach experience are your best tiebreakers.' }], thisYear: '8 vs 9 is the hardest matchup to predict. Just pick based on tempo and coaching.' },
              ].map(({ matchup, record, color, scenarios, thisYear }) => (
                <div key={matchup} style={{ ...S.card, marginBottom: 20, borderColor: `${color}44` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                    <div style={{ background: color, color: '#fff', borderRadius: 8, padding: '6px 14px', fontSize: 16, fontWeight: 800, fontFamily: "'Playfair Display', serif", flexShrink: 0 }}>{matchup}</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
                    {scenarios.map((s, i) => (
                      <div key={i} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: '10px 14px', border: '1px solid rgba(255,255,255,0.07)' }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color, marginBottom: 4 }}>⚠️ {s.label}</div>
                        <div style={{ fontSize: 13, color: '#ccc', marginBottom: 4 }}>{s.detail}</div>
                        <div style={{ fontSize: 11, color: '#888', fontStyle: 'italic' }}>💡 {s.tip}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ padding: '10px 14px', background: `${color}15`, borderRadius: 8, border: `1px solid ${color}33`, fontSize: 13, color: '#ccc' }}>
                    ...
                    {studentBoard.length === 0
                      ...
                      : studentBoard.map((e, i) => (
                        <div key={e.uid} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 12px', background: e.uid === user?.uid ? 'rgba(22,163,74,0.08)' : 'transparent', borderRadius: 8, marginBottom: 3, border: e.uid === user?.uid ? '1px solid rgba(22,163,74,0.25)' : '1px solid transparent' }}>
                          ...
                          {e.photoURL ? <img src={e.photoURL} alt={e.displayName} width={26} height={26} style={{ borderRadius: '50%' }} /> : <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#111', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#777' }}>?</div>}
                    ...
                    <div style={{ ...S.card, marginTop: 20, borderColor: 'rgba(245,158,11,0.25)' }}>
                      ...
                      {teacherBoard.map((e, i) => (
                        <div key={e.uid} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 12px', background: e.uid === user?.uid ? 'rgba(245,158,11,0.06)' : 'transparent', borderRadius: 8, marginBottom: 3, border: e.uid === user?.uid ? '1px solid rgba(245,158,11,0.2)' : '1px solid transparent' }}>
                          ...
                          {e.photoURL ? <img src={e.photoURL} alt={e.displayName} width={26} height={26} style={{ borderRadius: '50%' }} /> : <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#111' }}>?</div>}
                          ...
                          <button onClick={() => handleViewBracket(e.uid, e.displayName, false)} disabled={loadingBracket === e.uid} style={{ ...S.btn('rgba(245,158,11,0.12)', GOLD2), padding: '3px 10px', fontSize: 11, border: `1px solid rgba(245,158,11,0.25)`, flexShrink: 0 }}>{loadingBracket === e.uid ? '...' : 'View'}</button>
                  ...
                  {mammalTeacherBoard.length > 0 && (
                    <div style={{ ...S.card, marginTop: 20, borderColor: 'rgba(245,158,11,0.25)' }}>
                      ...
                      {mammalTeacherBoard.map((e, i) => (
                        <div key={e.uid} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 12px', background: e.uid === user?.uid ? 'rgba(245,158,11,0.06)' : 'transparent', borderRadius: 8, marginBottom: 3 }}>
                          <span style={{ fontSize: 17, fontWeight: 700, color: i === 0 ? GOLD2 : '#666', minWidth: 30, fontFamily: "'Playfair Display', serif" }}>#{i+1}</span>
                          {e.photoURL ? <img src={e.photoURL} alt={e.displayName} width={26} height={26} style={{ borderRadius: '50%' }} /> : <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#111' }}>?</div>}
                          <span style={{ flex: 1, fontWeight: e.uid === user?.uid ? 700 : 400, color: e.uid === user?.uid ? GOLD2 : '#bbb', fontSize: 14 }}>{formatName(e.displayName)}{e.uid === user?.uid ? ' (You)' : ''}</span>
                          <button onClick={() => handleViewBracket(e.uid, e.displayName, true)} disabled={loadingBracket === e.uid + '-mm'} style={{ ...S.btn('rgba(245,158,11,0.12)', GOLD2), padding: '3px 10px', fontSize: 11, border: `1px solid rgba(245,158,11,0.25)`, flexShrink: 0 }}>{loadingBracket === e.uid + '-mm' ? '...' : 'View'}</button>
                          <span style={{ fontSize: 20, fontWeight: 700, color: GOLD2, fontFamily: "'Playfair Display', serif" }}>{e.score}</span>
                        </div>
                      ))}
                    </div>
          ...
          {tab === 'admin' && isAdmin && (
            <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
              ...
              {adminSubTab === 'dashboard' && (
                <>
                  ...
                  <div style={{ ...S.card, borderColor: 'rgba(22,163,74,0.3)', marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <input type="number" value={yearDraft} onChange={e => setYearDraft(e.target.value)} aria-label="Tournament year" style={{ ...S.input, width: 110, padding: '8px 12px', fontSize: 16 }} />
              ...
              {adminSubTab === 'teams' && (
                <>
                  <TeamEntryPanel onTeamsSaved={handleTeamsSaved} onRequestGenerateResearch={handleGenerateResearch} regionNames={bbRegionNames} onRegionNamesChange={handleSaveBbRegionNames} sourcesData={bbSources} onSaveSources={handleSaveBbSources} />
                  {Object.keys(researchData).length > 0 && (
                    <div style={{ ...S.card, marginBottom: 16, borderColor: 'rgba(245,158,11,0.25)' }}>
                      ...
                      <p style={{ color: '#777', fontSize: 12, marginBottom: 14 }}>Delete individual research entries that have stale or incorrect data.</p>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {Object.keys(researchData).sort().map(name => (
                          <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)' }}>
                            ...
                            <button onClick={() => setConfirmDialog({ message: `Delete research entry for "${name}"?`, onConfirm: async () => { setConfirmDialog(null); const updated = { ...researchData }; delete updated[name]; await saveResearchData(updated); setResearchData(updated); } })}
                              style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: 14, padding: '0 2px', lineHeight: 1 }} aria-label={`Delete ${name}`}>×</button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: 40, borderTop: '1px solid rgba(239,68,68,0.2)', paddingTop: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                      {[['Clear Basketball Roster','Deletes the team roster and official bracket. Use at the start of a new season.',() => setConfirmDialog({ message: 'Delete the basketball roster and official bracket? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearBbRoster(); } })],['Clear Basketball Research','Deletes all scouting reports.',() => setConfirmDialog({ message: 'Delete all basketball research cards? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearBbResearch(); } })],['Clear All User Brackets','Deletes every student and teacher basketball bracket and resets the leaderboard.',() => setConfirmDialog({ message: 'Delete ALL user basketball brackets and leaderboard scores? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearAllBrackets(false); } })]].map(([title, desc, action]) => (
                        <div key={title} style={{ ...S.card, borderColor: 'rgba(239,68,68,0.25)', flex: 1, minWidth: 220 }}>
                          <h4 style={{ color: '#f87171', marginBottom: 6 }}>{title}</h4>
                          <p style={{ color: '#777', fontSize: 12, marginBottom: 12 }}>{desc}</p>
                          <button style={{ ...S.btn('#7f1d1d', '#fca5a5'), padding: '7px 16px', fontSize: 12, border: '1px solid rgba(239,68,68,0.4)' }} onClick={action}>
                            {title.replace('Clear ','Clear ').replace('Roster','Roster & Bracket')}
                          </button>
                        </div>
                      ))}
                    </div>
              ...
              {adminSubTab === 'mammals' && (
                <>
                  ...
                  <div style={{ marginTop: 40, borderTop: '1px solid rgba(239,68,68,0.2)', paddingTop: 24 }}>
                    <div style={{ fontSize: 11, color: '#e74c3c', letterSpacing: 2, textTransform: 'uppercase', fontWeight: 700, marginBottom: 16 }}>⚠️ Danger Zone</div>
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                      {[['Clear Mammal Roster','Deletes the animal roster and official mammal bracket.',() => setConfirmDialog({ message: 'Delete the mammal roster and official bracket? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearMammalRoster(); } })],['Clear Mammal Research','Deletes all organism profiles and images.',() => setConfirmDialog({ message: 'Delete all mammal research cards and images? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearMammalResearch(); } })],['Clear All User Brackets','Deletes every student and teacher mammal bracket and resets the mammal leaderboard.',() => setConfirmDialog({ message: 'Delete ALL user mammal brackets and leaderboard scores? This cannot be undone.', onConfirm: async () => { setConfirmDialog(null); await handleClearAllBrackets(true); } })]].map(([title, desc, action]) => (
                        <div key={title} style={{ ...S.card, borderColor: 'rgba(239,68,68,0.25)', flex: 1, minWidth: 220 }}>
                          <h4 style={{ color: '#f87171', marginBottom: 6 }}>{title}</h4>
                          <p style={{ color: '#777', fontSize: 12, marginBottom: 12 }}>{desc}</p>
                          <button style={{ ...S.btn('#7f1d1d', '#fca5a5'), padding: '7px 16px', fontSize: 12, border: '1px solid rgba(239,68,68,0.4)' }} onClick={action}>{title}</button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
                    <div>
                      ...
                      <p style={{ color: '#777', fontSize: 13, margin: 0 }}>{allUsers.length} registered user{allUsers.length !== 1 ? 's' : ''}. Removing a user deletes their brackets and scores but not their Google account.</p>
                    </div>
                  </div>
                  {allUsers.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {allUsers.sort((a, b) => (a.displayName || '').localeCompare(b.displayName || '')).map(u => (
                        <div key={u.uid} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.07)' }}>
                          {u.photoURL ? <img src={u.photoURL} alt={u.displayName} width={32} height={32} style={{ borderRadius: '50%', flexShrink: 0 }} /> : <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#888', flexShrink: 0 }}>{(u.displayName || '?').charAt(0)}</div>}
                          <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: '#444', marginRight: 8 }}>{u.lastSeen?.seconds ? new Date(u.lastSeen.seconds * 1000).toLocaleDateString() : ''}</div>
                          <button onClick={() => setConfirmDialog({ message: `Remove ${u.displayName || 'this user'}? This deletes their brackets and scores but not their Google account.`, onConfirm: async () => { setConfirmDialog(null); await handleRemoveUser(u.uid); } })}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ ...S.card, borderColor: 'rgba(99,102,241,0.25)' }}>
                    ...
                    <p style={{ color: '#888', fontSize: 14, lineHeight: 1.75 }}>
                      ...
                      To update your Claude API key: go to Vercel → your project → Settings → Environment Variables → update <code style={{ background: 'rgba(255,255,255,0.07)', padding: '1px 5px', borderRadius: 3 }}>ANTHROPIC_KEY</code>.
                    </p>
                  </div>
